...
[run_time_OBOFoundry.R](run_time_OBOFoundry.R).

There are two objects in `"runtime_OBOFoundry_all.RData"`: `lt` which is a
list of two-column data frames which contains number of random terms to test
and corresponding runtime, `df` a data frame that contains meta-information of
each ontology.
...
Different ontologies have different range of runtime. To make them comparible,
...
area difference of the curve to `y = x` (the diagonal line). If the curve bends
more to the bottom left of the plotting region, it means the it has higher time complexity.
