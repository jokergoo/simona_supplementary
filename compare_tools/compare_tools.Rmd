...
We have to manually update the IC values that are internally used. Now the internal ICs are
the same as in `IC`:
...
```{r, fig.width = 10, fig.height = 5, fig.cap = "Figure S1-3. Systematic shift of IC values from simona to GOSemSim. Left: over-estimatation on the IC values with relation to the depth. Right: number of terms on each depth."}
depth = dag_depth(dag)
...
```{r, fig.width = 10, fig.height = 5, fig.cap = "Figure S1-6. Systematic shift of semantic similarity values from simona to GOSemSim. Left: over-estimatation on the similarity values with relation to the depth of MICA terms. Right: number of MICA terms on each depth. A depth of zero means the MICA term is the root term."}
...
barplot(table(df$depth), xlab = "MICA Depth", ylab = "Counts")
```
...
```{r, fig.width = 6, fig.height = 6, fig.cap = "Figure S1-7. Runtime of simona, ontologySimilarity, GOSemSim and GOSim on calculating semantic similarities of the random 500 GO BP terms."}
