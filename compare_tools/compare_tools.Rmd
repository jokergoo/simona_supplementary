...
**GOSemSim** and **GOSim** calculate identical IC values, as can be validated
...
According to the two different ways to calculate gene annotation-based IC, the
...
# the second plot
plot(NULL, xlim = c(0, 9000), ylim = c(0, 30), 
	xlab = "Number of terms", ylab = "runtime (sec)",
...
In the following plot, when numbers of terms are small, **ontologySimilarity** has better
runtime performance than **simona**, mainly because **simona** has extra computations before
traversing the DAG to look for MICA terms, which becomes a major factor when the numbers of terms
are small. But the performance of **simona** improves when the number of terms increases.
...
plot(k, t4/t1, xlab = "Number of terms", ylab = "GOSim / simona", main = "Runtime fold improvement")
```

To easy compare their time complexities, we scale numbers of random terms as well as
the runtime both into the scale of `[0, 1]`. We also add the lines for linear, quadratic and cubic
time complexities in the next plot.


```{r, fig.width = 6, fig.height = 6, fig.cap = "Figure S1-9. Relative time complexity of simona, ontologySimilarity and GOSemSim."}
rel_k = k/max(k)
rel_t1 = t1/max(t1)
rel_t2 = t2/max(t2)
rel_t3 = t3/max(t3)
...
plot(NULL, xlim = c(0, 1), ylim = c(0, 1),
	...
	main = "Compare relative time complexity")
lines(rel_k, rel_t1, col = 2)
lines(rel_k, rel_t2, col = 3)
lines(rel_k, rel_t3, col = 4)

curve(x^1, lty = 1, add = TRUE)
curve(x^2, lty = 2, add = TRUE)
curve(x^3, lty = 3, add = TRUE)

legend("topleft", lty = c(1, 1, 1, 1, 2, 3), col = c(2, 3, 4, 1, 1, 1), 
	legend = c("simona", "ontologySimilarity", "GOSemSim", "O(n)", "O(n^2)", "O(n^3)"))
```

From the plot above, it is straightforward to see **simona** has a time complexity close
to $O(n^2)$, while **ontologySimilarity** and **GOSemSim** have time
complexity close to $O(n^3)$.
