...
```{r}
head(IC)  # the one we just generated
head(head(GOSimEnv$IC)) # the one internally used
GOSim::setOntology("BP", DIR = ".")
head(head(GOSimEnv$IC)) # the one we have updated
```
...
**GOSemSim** and **GOSim** calculate identical IC values. This is
...
genes annotated to the root of the GO DAG).
...
the IC values. The two types of methods are the same when a gene is only
...
and **GOSim** generates identical results. **GOSemSim** and **GOSim** estimate systematically higher
similarities than **simona** and **ontologySimilarity**, due to that they two over-estimate
the ICs.
