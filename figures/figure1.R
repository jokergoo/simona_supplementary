...
p1 = ggplot(df, aes(x = simona, y = ontologyIndex)) +
	geom_point(size = 1) +
	ggtitle("Compare ICs between simona / ontologyIndex")
p2 = ggplot(df, aes(x = simona, y = GOSemSim)) +
	...
	ggtitle("Compare ICs between simona / GOSemSim")
p3 = ggplot(df, aes(x = GOSim, y = GOSemSim)) +
	geom_point(size = 1) +
	scale_x_continuous(limits = c(0, 12.5), breaks = seq(0, 12, by = 4)) +
	scale_y_continuous(limits = c(0, 12.5), breaks = seq(0, 12, by = 4)) +
	ggtitle("Compare ICs between GOSim / GOSemSim")
...
p4 = grid.grabExpr(
	draw(Heatmap(m1, name = "similarity", col = col_fun,
		...
		column_title = "Similaritysimona, random 500 GO terms")
	), width = 4, height = 4
)
p5 = grid.grabExpr(
...
p8 = ggplot(df1, aes(x = k, y = runtime, col = tool)) +
	...
	ggtitle("Compare runtime performance")
...
pdf("figure1.pdf", width = 13, height = 13)
print(plot_grid(
