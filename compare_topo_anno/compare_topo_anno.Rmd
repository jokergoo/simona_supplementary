...
```{r, fig.width = 5, fig.height = 5, fig.cap = "Figure S5-1. Sub-DAG of ancestors of two GO terms of GO:0010035 and GO:1901701."}
...
in the DAG a larger value of of depth a term has (the depth of a parent is
...
compare the relations the global IC_annotation and depth.
...
par(mfrow = c(1, 2))
# because depth is discrete, we use boxplot to visualize the correlation
...
Finally, Let's take a simplified example to show the relations between IC_annotation and depth.
Consider an ontology with a binary tree structure, i.e. each term has two children and each term
has an unique gene annotated. All the leaf terms in the ontology have depth of $d$. Then the total number
of genes annotated to the root is $2^{d+1}-1$. For a term with height $h (0 \leq h \leq d)$, the number
of genes annotated is $2^{d-h+1}-1$, then the IC_annotation would be:

$$ 
\begin{align*}
\mathrm{IC}_\mathrm{annotation} & = -\log\left(\frac{2^{d-h + 1} - 1}{2^{d+1} - 1} \right) \\
            & \approx  -\log\left(\frac{2^{d-h + 1}}{2^{d+1}} \right) \\
            & = -\log\left(2^{-h} \right ) \\
            & = h * \log2
\end{align*} $$
